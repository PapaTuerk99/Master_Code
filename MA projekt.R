...
#plot(adj_plot,layout=layout, edge.width=0.5, vertex.size=10, main = "Laplace Clustering" ,vertex.color=j$cluster)
...
for(ä in 1:10){
  ...
  abc[[1]]<-c(0,0,0)
  abc[[2]]<-c(a,b,c)
  # Zusätzlich Gamma selber schätzen für a;b=1 
  
  #exp<-expected_degree_from_adj_matrices(dynamic_networks, 1)
  gamma<-1
  for (ü in 3:30){
    differenz[[ü]] <- abc[[ü-1]] - abc[[ü - 2]]
    ap<-euklidische_norm(differenz[[ü]])
    if(ap>0.0001){
      ...
      Wij<-b_est*sum_pers+ a_est*sum_neu+c_est*sum_surv
      #gamma<- log(Q00/P00)*(n^2/(n-1))*(1/((a_est+b_est)*exp))
      gamma<-1
      abc[[ü]]<-c(a_est, b_est, c_est)
    }
    else {
...
# Parameter festlegen
  ...
  abc[[1]]<-c(0,0,0)
  abc[[2]]<-c(a,b,c)
  # Zusätzlich Gamma selber schätzen für a;b=1. Bei uns gamma=1. 
  gamma<-1
  for (ü in 3:30){
    differenz[[ü]] <- abc[[ü-1]] - abc[[ü - 2]]
    ap<-euklidische_norm(differenz[[ü]])
...
# Zusatz
################################################################################################
# Beispiel anhand eines Aktiengraphen (einzelne Aktien des DAX)
# Kante, falls Korrelation größer als 0,4 ist.
# Wir untersuchen die ersten fünf Wochen von 2024:
# DAX Ticker:
dax_tickers <- c(
...
M6<-A_dax_list[[6]]
M7<-A_dax_list[[7]]
vemdax <-BM_bernoulli_multiplex("SBM", list(M1,M2,M3,M4,M5))
vemdax$estimate()
